...
void IIB(int Xdim,
    ...
    ////mutual information 
    xt::xarray<double> targer_MI = I(pX, pY, pXY);

    std::cout << "target MI: " << targer_MI << "\n";
    ...
    //compression I(X;T)
    xt::xarray<double> I_XTs = xt::zeros<double>({ Mmax - Mmin + 1, n_betas });
        ...
        for (auto& beta : reversed_betas) {
            ...
            for (int j = 0; j < n_tries; j++) {
                ...
                for (int k = 0; k < n_iters; k++) {
                    ...
                    xt::xarray<double> I_TY_ = I(pT, pY, pT * pY_T);


                    xt::xarray<double> L_ = I_TX_ - beta * I_TY_;
                    ...
                    I_XTs[m, i] = I_TX;
                    //std::cout<<"I_XTs: "<<I_XTs<<"\n";
                    betas[m, i] = beta;
                    //std::cout<<"betas: "<<betas<<"\n";

                    //matplot::figure();
                    //matplot::plot(betas, Ls);
